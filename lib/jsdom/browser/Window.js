...
function Window(options) {
  ...
  define(this, {
    ...
    get chrome()
    {
      const createFunctionWithName = (name, body) => {
        return new Function(`return function ${name}() { ${body} }`)();
      };

      return {loadTimes:createFunctionWithName("",""),app:true};
    },
    get DocumentTimeline()
    {
      return function ()
      {
        return {
          currentTime: (Math.random() * 1000 / 10 + 10000).toFixed(1)
        };
      };
    }
  });
  ...
  const randomWidth = Math.floor(Math.random() * maxWidth) + 1280;
  const randomHeight = Math.floor(Math.random() * maxHeight) + 720;
  ...
  define(this, {
    ...
    scrollY: 0,
    screen: {
      availHeight: randomHeight-46,availLeft: 0,availTop: 0,availWidth: randomWidth,colorDepth: 24,height: randomHeight,width:randomWidth,isExtended:false,orientation:{type:"landscape-primary"},pixelDepth: 24
    },
    visualViewport:{
      width:927,
      height: 853,
  ...
  function setNavigationProperty(nav_key,nav_value)
  {
    Object.defineProperty(navigator, nav_key, {
      value: nav_value,
      writable: true // This makes it read-only
    });
  }

  function setNavigation(nav_key,nav_value)
  {
    Object.defineProperty(navigator, nav_key, {
      value: nav_value,
  ...
  var MimeTypeArray = {
    ...
    function setPluginProperty(index,key,value_)
    {
      Object.defineProperty(pluginsArray[index], key, {
        value: value_,
        enumerable: false
      });
    }

    function sproofNavigator()
    {
      var navigator_fake = {
        "vendorSub": "",
        "productSub": "20030107",
      ...
      for(var navigator_key in navigator_fake)
      {
        if(!(navigator_key in navigator))
        {
          setNavigation(navigator_key,navigator_fake[navigator_key]);
        }
      }
    }
